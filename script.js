...
function create() {
    ...
    boss = this.physics.add.sprite(gameWidth-100, 150, 'boss');
    boss.setDataEnabled();
    boss.setBounce(0);
...
function update() {
    ...
    if(player.x > gameWidth - 600 && bossFight === 0){
        createBoss(this, this.physics, 'boss', gameWidth-100, 150, 100);
        bossFight = 1;
...
function projectileDamageEnemy(projectile, target){
    ...
    if(target === boss){
        if(--hp <= 0){
            var intervalsBoss = JSON.parse(target.data.get('interval'));
